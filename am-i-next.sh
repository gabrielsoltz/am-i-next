...
print_summary() {
    ...
    if [[ "${FINDINGS}" -gt 0 ]]; then
        ...
        echo "    2. Move secrets into a secrets manager (1Password, Vault, etc.)."
        echo "    3. Use short-lived credentials / OIDC where possible."
        echo "    4. Enable MFA everywhere and use hardware keys (FIDO2/Passkeys)."
        echo "    5. Audit git history: trufflehog git file://\$PWD"
    else
        echo -e "  ${GREEN}${BOLD}No findings — looking clean!${NC}"
        echo ""
        echo -e "  ${DIM}Tip: adjust --results in config.conf to surface unverified candidates too.${NC}"
    fi
    echo ""
...
main() {
    ...
    [[ -n "${JSON_OUTPUT_FILE}" ]] && info "Raw JSON → ${JSON_OUTPUT_FILE}"
